...
class MessageHandler:
    ...
    async def handle_custom_time(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            BotLogger.log_user_action(
                user_id, username, f"custom_time_input: {time_text}"
            )
            ...
            if self.user_settings.update_user_setting(
                user_id, "schedule_time", time_text
            ):
                await message.reply(
            ...
            await message.reply(
                "❌ Произошла ошибка. Попробуйте еще раз.", parse_mode="Markdown"
            )
    ...
    async def handle_page_number(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            # Логируем действие пользователя
            BotLogger.log_user_action(
                user_id, username, f"page_number_input: {page_text}"
            )
