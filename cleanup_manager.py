...
class CleanupManager:
    ...
    def cleanup_old_images(
        ...
        if retention_days is None:
            retention_days = get_config().cleanup_older_than_days
    ...
    def get_storage_usage() -> dict:
        ...
        try:
            ...
            if os.path.exists(get_config().output_dir):
                for root, dirs, files in os.walk(get_config().output_dir):
                    for file in files:
                        file_path = os.path.join(root, file)
                        try:
            ...
            if os.path.exists(get_config().upload_dir):
                for root, dirs, files in os.walk(get_config().upload_dir):
                    for file in files:
                        file_path = os.path.join(root, file)
                        try:
                            size = os.path.getsize(file_path)
